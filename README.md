...
The current Phase-2 flip-flop, static-MOS semiconductor, and Model-85 cache bridges are all `grounded`. Case 07 supports `static/no refresh ≠ unpowered`, retention-supporting supply distinct from ordinary active operation, and **cell-state retention ≠ reliable selection/sensing/recovery**. Case 08 adds a separate architectural distinction: **cache-copy retention ≠ authoritative-state retention**. In the bounded Model 85, program-visible main-storage addresses remain stable while derivative fast-copy residence changes; sector correspondence and block validity decide whether cache payload is admissible/current, and replacement may discard that copy without losing main-storage state. Later write-back/dirty-state and multiprocessor coherence remain separate cases rather than being back-projected into 1968 terminology. See [`CASE_INDEX.md`](CASE_INDEX.md) for authoritative maturity status and [`ROADMAP.md`](ROADMAP.md) for the next slice.
