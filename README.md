...
The current post-audit technical bridge set now extends through Case 11. Cases 09–10 show that a DRAM retention deadline can remain while row enumeration, trigger cadence, and scheduling responsibility move across the package boundary. Case 11 adds a different semiconductor constraint: **nonvolatility ≠ immutability**, **electrical programmability ≠ electrical erasability**, and **read/program addressability ≠ erase geometry**. The ROM/PROM/EPROM/EEPROM/Flash roadmap item therefore remains open: the next bounded step is an EEPROM case in which erasure becomes electrically controlled, not an unsupported jump from EPROM directly to Flash. See [`CASE_INDEX.md`](CASE_INDEX.md) for authoritative maturity status and [`ROADMAP.md`](ROADMAP.md) for the next slice.
