...
The current Phase-2 flip-flop bridge and static-MOS semiconductor bridge are both `grounded`. Case 07 now supports `static/no refresh ≠ unpowered`, retention-supporting supply distinct from ordinary active operation, and a second separation: **cell-state retention ≠ reliable selection/sensing/recovery**. Exact 1101/1101A/2102/2102A product topology and Vadasz–Chua–Grove 1971 facsimile inspection remain archival cleanup rather than central mechanism blockers. The next bounded Phase-2 task is cache policy/interface semantics — tag/data identity, hit/miss admissibility, replacement/invalidation, dirty state, and write policy — without reopening generic SRAM-cell history. See [`CASE_INDEX.md`](CASE_INDEX.md) for authoritative maturity status and [`ROADMAP.md`](ROADMAP.md) for the next slice.
