...
- [`cases/07-static-mos-ram-powered-quiescence.md`](cases/07-static-mos-ram-powered-quiescence.md) — first-pass semiconductor-array stress test: period static-MOS / flip-flop vocabulary, no-refresh powered quiescence, decoded-array semantics, nondestructive read in bounded Intel devices, and the 5101L distinction between retention-supporting supply and ordinary active operation; cache remains deferred.
...
The current Phase-2 flip-flop bridge is `grounded`, while [`cases/07-static-mos-ram-powered-quiescence.md`](cases/07-static-mos-ram-powered-quiescence.md) opens the next semiconductor bridge at `first-pass`. Its period package-level evidence already supports `static/no refresh ≠ unpowered`, decoded-array organization, and a lower-voltage retention condition distinct from ordinary active operation in the 5101L family. The next bounded task is source deepening for the actual Intel static bit cell and direct inspection of the Vadasz–Chua–Grove 1971 pages before cache policy/interface semantics are added. See [`CASE_INDEX.md`](CASE_INDEX.md) for authoritative maturity status and [`ROADMAP.md`](ROADMAP.md) for the next slice.
