...
- [`Synthesis 26 deepening — typed reset events, state-class persistence, and evidence closure`](../evidence/synthesis26-reset-event-state-persistence-horizon-deepening.md) — grounded Cases 85, 148, and 38 show that `reset survival` is not one scalar durability property. Micron L83A `89h` survives named NAND resets while in-flight work is aborted; NVMe extended Device Self-test carries a continuation obligation across Controller Level Reset without specifying exact resume microstate; and S3700 `D000h` sits inside a protocol with persistent-option machinery while named-feature support remains unproved. The bounded result is `same word reset != same event boundary`, and a safe persistence claim must bind **state class × event class × interface layer × survival semantics × evidence layer**.
