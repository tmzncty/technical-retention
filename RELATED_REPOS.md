...
Case 05 now keeps two bounded control/currentness seams in this repository. [`evidence/05-crush-2006-map-epoch-placement-currentness-deepening.md`](evidence/05-crush-2006-map-epoch-placement-currentness-deepening.md) isolates **stable object/PG designation → current cluster-map/epoch context → CRUSH placement policy → current intended target relation → separate peering/content-currentness qualification → migration/recovery obligation**. [`evidence/05-ceph-2007-osdmap-restart-persistence-deepening.md`](evidence/05-ceph-2007-osdmap-restart-persistence-deepening.md) adds the source-level restart seam **persisted full/incremental OSDMaps + persisted superblock epoch selection/range + persisted PG metadata/logs → restart-time local placement/history context → missing/newer map reconciliation → separate peering/content-currentness qualification**. Fresh companion searches for `RADOS` and `CRUSH` found no dedicated packet to reuse. Broad RUSH→CRUSH genealogy, the exact 2005 RG/RUSH→2007 CRUSH/PG implementation bridge, monitor-side map-governance/persistence history, EBOFS/ObjectStore crash-ordering archaeology, production deployment, map-trimming evolution, and later `up`/`acting`/backfill semantics remain `computing-archaeology` work rather than being duplicated here.
