...
evidence_path = Path('evidence/142-ceph-reef-source-retry-state-horizon-deepening.md')
if evidence_path.exists():
    raise SystemExit(f'{evidence_path} already exists')
evidence_path.write_text(r'''# Evidence 142B — Ceph Reef source-level capacity-gated retry state and retention horizons
...
case_path = 'cases/142-ceph-reef-capacity-gated-recovery.md'
old = "The detailed source ledger is [`../evidence/142-ceph-reef-capacity-gated-recovery-grounding.md`](../evidence/142-ceph-reef-capacity-gated-recovery-grounding.md)."
...
replace_once(case_path, old, new)

anchor = "This is the core contribution of the case.\n\n---\n\n## Prospective capacity: already-mapped work can reserve the future"
insertion = r'''This is the core contribution of the case.
...
## Prospective capacity: already-mapped work can reserve the future'''
replace_once(case_path, anchor, insertion)

old = "- the Reef documentation does not establish exact behavior for every mClock or scheduler configuration.\n- no throughput, latency, or data-loss probability is inferred without measurement."
new = "- the Reef source deepening does not establish that `_TOOFULL` PG flags themselves survive daemon restart.\n- a scheduled retry does not guarantee eventual progress; unchanged fullness can gate the next attempt again.\n- the Reef documentation/source slice does not establish exact behavior for every mClock or scheduler configuration.\n- upstream standalone QA is intentional project-test evidence, not a named production trace.\n- no throughput, latency, or data-loss probability is inferred without measurement."
replace_once(case_path, old, new)

old = "- `computing-archaeology`: first-introduction genealogy of fullness ratios/reservations, source-code state-machine evolution, scheduler history, release-by-release behavior, and implementation benchmarks if developed later."
...
old = "- Related evidence ledger: [`../evidence/142-ceph-reef-capacity-gated-recovery-grounding.md`](../evidence/142-ceph-reef-capacity-gated-recovery-grounding.md)."
...
grounding = Path('evidence/142-ceph-reef-capacity-gated-recovery-grounding.md')
gtext = grounding.read_text()
follow = r'''
...
'''
if '## Follow-up — Reef `v18.2.0` source-level retry-state deepening' not in gtext:
    grounding.write_text(gtext.rstrip() + follow + '\n')
else:
    raise SystemExit('grounding follow-up already present')

roadmap = Path('ROADMAP.md')
rtext = roadmap.read_text()
patt = re.compile(r'^- \[x\] Case 142 Ceph Reef capacity-gated recovery / repair-headroom slice — .*$', re.M)
hits = patt.findall(rtext)
if len(hits) != 1:
    raise SystemExit(f'ROADMAP Case 142 bullet count={len(hits)}')
replacement = "- [x] Case 142 Ceph Reef capacity-gated recovery / repair-headroom slice — [`cases/142-ceph-reef-capacity-gated-recovery.md`](cases/142-ceph-reef-capacity-gated-recovery.md) + [`evidence/142-ceph-reef-capacity-gated-recovery-grounding.md`](evidence/142-ceph-reef-capacity-gated-recovery-grounding.md) + [`evidence/142-ceph-reef-source-retry-state-horizon-deepening.md`](evidence/142-ceph-reef-source-retry-state-horizon-deepening.md): Reef documentation separates `active` from `clean`, ordinary wait from `*_toofull`, projected occupancy from current occupancy, and cluster fullness policy from explicit operator suspension. The released `v18.2.0` state machine now closes the broad source-level baseline: too-full recovery/backfill reactions set per-PG episode state, unwind or leave the current reservation attempt, and schedule `DoRecovery()` / `RequestBackfill()` after separately configurable retry intervals. This grounds distinct policy, episode, and retry horizons while preserving `gate active != work abandoned`, `PG flag cleared != policy changed`, and `retry scheduled != progress guaranteed`. Remaining work is narrower: first-introduction/pre-post-Reef evolution, mClock/work-class interaction, named production traces, and controlled capacity/fault experiments."
roadmap.write_text(patt.sub(replacement, rtext, count=1))

index = Path('CASE_INDEX.md')
itext = index.read_text()
patt = re.compile(r'^\| \[Ceph Reef Capacity-Gated Recovery: Repair Headroom, Admission, and Redundancy Debt\]\(cases/142-ceph-reef-capacity-gated-recovery\.md\) \|.*$', re.M)
hits = patt.findall(itext)
if len(hits) != 1:
    raise SystemExit(f'CASE_INDEX Case 142 row count={len(hits)}')
replacement = "| [Ceph Reef Capacity-Gated Recovery: Repair Headroom, Admission, and Redundancy Debt](cases/142-ceph-reef-capacity-gated-recovery.md) | **grounded** | surviving RADOS replicas + PG degraded/clean state + OSDMap fullness policy + per-PG `*_toofull` episode state + delayed retry events | separate serviceability, redundancy debt, destination admission, cluster policy, episode reporting, and retry obligation; show a rejected repair embodiment can end while the reason to retry remains live | [Reef documentation grounding](evidence/142-ceph-reef-capacity-gated-recovery-grounding.md) + [v18.2.0 source retry-state deepening](evidence/142-ceph-reef-source-retry-state-horizon-deepening.md); first-introduction/pre-post-Reef state-machine evolution, mClock interaction, named-cluster traces, and controlled fault/capacity experiments remain open |"
index.write_text(patt.sub(replacement, itext, count=1))

for helper in [Path('.github/workflows/case142-source-retry-deepening.yml'), Path('scripts/case142_source_retry_deepening.py')]:
    if helper.exists():
        helper.unlink()
