...
synthesis = Path('docs/SYNTHESIS_28_RECLAMATION_AUTHORITY_AFTER_RETIREMENT.md')
...
required = [
    ...
    Path('docs/TECHNICAL_SPINE.md'),
    Path('RELATED_REPOS.md'),
    Path('docs/SYNTHESIS_22_ERASE_INVALIDATION_SANITIZATION_VERIFICATION.md'),
    Path('docs/SYNTHESIS_24_RETENTION_MAINTENANCE_TRIGGER_REGIMES.md'),
    Path('cases/73-google-gfs-lazy-garbage-collection.md'),
    Path('cases/125-linux-ext3-ext4-orphan-crash-cleanup-reclamation.md'),
    Path('cases/145-jffs2-garbage-collection-negative-state-evidence.md'),
    Path('cases/147-s3-multipart-upload-preobject-retention.md'),
    Path('cases/150-crucial-m550-active-garbage-collection.md'),
    Path('cases/153-ceph-rados-snaptrim-asynchronous-reclamation.md'),
    Path('cases/44-nvme13-deallocate-sanitize-forgetting.md'),
    Path('cases/47-fast11-ssd-sanitization-verification.md'),
    Path('evidence/73-gfs-2003-lazy-garbage-collection-grounding.md'),
    Path('evidence/125-linux-ext3-ext4-2000-2023-orphan-recovery-grounding.md'),
    Path('evidence/145-jffs2-2001-garbage-collection-negative-state-grounding.md'),
    Path('evidence/147-s3-2010-2016-multipart-preobject-retention-grounding.md'),
    Path('evidence/150-crucial-2014-2024-active-garbage-collection-grounding.md'),
    Path('evidence/153-ceph-2013-snaptrim-asynchronous-reclamation-grounding.md'),
]
for p in required:
    if not p.exists():
        raise SystemExit(f'missing expected prerequisite: {p}')

if synthesis.exists():
    raise SystemExit(f'refusing to overwrite existing synthesis: {synthesis}')

text = r"""# Synthesis 28 — Reclamation Authority After Retirement: Eligibility, Cleanup Obligations, Reuse, and Sanitization
...
# README navigation: append immediately after the latest existing synthesis entry.
s = readme.read_text(encoding='utf-8')
new_path = 'docs/SYNTHESIS_28_RECLAMATION_AUTHORITY_AFTER_RETIREMENT.md'
if new_path not in s:
    anchors = [i for i, line in enumerate(s.splitlines()) if 'SYNTHESIS_27_RETENTION_POLICY_EVIDENCE_VALIDITY.md' in line]
    if not anchors:
        raise SystemExit('README: Synthesis 27 navigation anchor not found')
    lines = s.splitlines()
    insert_at = max(anchors) + 1
    lines.insert(insert_at, '- [`Synthesis 28 — Reclamation Authority After Retirement`](docs/SYNTHESIS_28_RECLAMATION_AUTHORITY_AFTER_RETIREMENT.md) — separates authority/reference retirement, reclamation eligibility, retained cleanup obligations, live-state preservation, cleanup completion, reuse admission, and sanitization across filesystem, object-store, Flash/SSD, and distributed-storage cases.')
    readme.write_text('\n'.join(lines) + ('\n' if s.endswith('\n') else ''), encoding='utf-8')

# CASE_INDEX: add a non-numbered routing block; synthesis creates no new historical findings.
s = index.read_text(encoding='utf-8')
heading = '### Cross-case synthesis — reclamation authority after retirement'
if heading not in s:
    ...
    block = '''### Cross-case synthesis — reclamation authority after retirement
...
# ROADMAP: record this bounded cross-case slice next to the existing reclamation coverage.
s = roadmap.read_text(encoding='utf-8')
road_line = '- [x] **Cross-case reclamation-authority synthesis** — distinguish authority/reference retirement, reclaim eligibility, retained cleanup obligations, live-state preservation, reclamation completion, reuse admission, and sanitization across GFS (73), ext3/ext4 orphan recovery (125), JFFS2 (145), S3 multipart abort (147), managed SSD GC (150), and Ceph snaptrim (153); route the cross-layer model through [`Synthesis 28`](docs/SYNTHESIS_28_RECLAMATION_AUTHORITY_AFTER_RETIREMENT.md).'
if 'SYNTHESIS_28_RECLAMATION_AUTHORITY_AFTER_RETIREMENT.md' not in s:
    lines = s.splitlines()
    candidates = [
        i for i, line in enumerate(lines)
        if ('73/125/145/150/153' in line) or
           ('Case 153' in line and ('garbage' in line.lower() or 'reclamation' in line.lower()))
    ]
    if not candidates:
        candidates = [i for i, line in enumerate(lines) if 'garbage collection' in line.lower() and 'Case 150' in line]
    if not candidates:
        raise SystemExit('ROADMAP: could not locate current reclamation coverage anchor')
    lines.insert(max(candidates) + 1, road_line)
    roadmap.write_text('\n'.join(lines) + ('\n' if s.endswith('\n') else ''), encoding='utf-8')

# Invariants: all navigation surfaces route to the synthesis and all canonical anchors still exist.
for p in [readme, roadmap, index]:
    t = p.read_text(encoding='utf-8')
    if new_path not in t:
        raise SystemExit(f'{p}: synthesis route missing after edit')
for p in required + [synthesis]:
    if not p.exists():
        raise SystemExit(f'expected linked file missing after edit: {p}')
PY

# Keep automation scaffolding out of the final research tree.
rm -f .github/workflows/add-synthesis-28-reclamation-authority.yml scripts/add_synthesis_28_reclamation_authority.sh

git add -A
git diff --cached --check

# The final research commit should contain only canonical research/navigation changes.
git diff --cached --name-status

git commit -m "synthesis28: distinguish retirement from reclamation authority"
