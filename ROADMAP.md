...
- [ ] latch / flip-flop / register — a bounded `first-pass` bridge exists in [`cases/06-flip-flop-powered-working-retention.md`](cases/06-flip-flop-powered-working-retention.md), with source-deepening ledger [`evidence/06-flip-flop-register-boundary-addendum.md`](evidence/06-flip-flop-register-boundary-addendum.md). Eccles–Jordan GB148582A and the 1946 ENIAC report establish regenerative powered state, explicit set/reset/clear, short-lived working-state use, and ENIAC's own `stores` / `remembers` vocabulary. The former period-`register` gap is now closed by Mann, Rathbone, and Bennett's 1954 Whirlwind R-221: it calls AR a `simple flip-flop register`, assigns it machine-level functions, and also uses `storage register` for core storage, showing that period `register` is an architectural/organizational term rather than one substrate class. Before marking the bridge complete, still directly inspect the 1919 Eccles–Jordan trigger-relay page/reprint and directly inspect PX-1-105 / relevant ENIAC Part-II circuit material; exact scan/drawing locators are now known, but locator evidence is not schematic/page inspection;
- [ ] SRAM and cache;
...
**Current technical stress test:** [`cases/06-flip-flop-powered-working-retention.md`](cases/06-flip-flop-powered-working-retention.md) remains `first-pass`, now with [`evidence/06-flip-flop-register-boundary-addendum.md`](evidence/06-flip-flop-register-boundary-addendum.md). Its primary evidence supports short-duration retention, continuous-power/refresh separation, later state-sensitive use without a discrete retrieval transaction, and now a period architectural boundary: Whirlwind R-221 explicitly describes AR as a `simple flip-flop register` while also using `storage register` for core memory, so state element, organization, architectural role, and substrate must be compared separately.

**Next highest-value unit:** finish Case 06's two remaining direct-inspection gaps rather than opening another broad case. The 1919 Eccles–Jordan *Electrician* p. 298 / *Radio Review* pp. 143–146 scan locations are now known, and ENIAC drawing PX-1-105 is identified as `Flip-Flop Circuit`; directly inspect those page/schematic images and record exact anchors. Promote Case 06 only if those inspections confirm the current mechanism reconstruction. The former period-`register` source gap is already closed by Whirlwind R-221 and should not be researched again from scratch.
