...
- [ ] ROM → PROM → EPROM → EEPROM → Flash — **EPROM and EEPROM bounded sub-slices are now grounded**. [`cases/11-intel-frohman-floating-gate-eprom-erasure.md`](cases/11-intel-frohman-floating-gate-eprom-erasure.md) separates trapped-charge nonvolatility, avalanche-injection programming, nondestructive lower-stress read, and radiation erase. [`cases/12-intel-2816-eeprom-electrical-erasure.md`](cases/12-intel-2816-eeprom-electrical-erasure.md), grounded by [`evidence/12-intel-1978-1981-eeprom-electrical-erasure-grounding.md`](evidence/12-intel-1978-1981-eeprom-electrical-erasure-grounding.md), moves erase into electrical/in-system control while preserving byte versus chip erase geometry, a distinct 21 V/timed erase-write regime, erase-before-write sequencing, and finite cycling. The broad lineage remains unchecked because the **device-level Flash transition to deliberately coarser/fast bulk erase** still needs its own period-primary case before being connected upward to already-grounded mapped Flash Case 04;
...
**Next highest-value unit:** remain on the ROM/PROM/EPROM/EEPROM/Flash bridge and source one bounded **early Flash erase-granularity transition**. Prefer a period-primary device paper, patent, or manufacturer source that directly explains why a coarser electrically erased unit / fast bulk erase is chosen, what unit is actually selected, and what density/speed/circuit-economy tradeoff it creates. Keep `Flash` as period terminology only where sourced. Then connect that device-level geometry upward to already-grounded Case 04's copy/reclaim/remap semantics rather than rewriting the FTL history.

The first generic thesis-audit sequence, four named philosophical/prior-art tests, the first category-coherence audit, and the post-audit technical stress tests are now grounded at bounded maturity. Do not promote the bounded relational criterion into a grand `What Is Technical Retention?` chapter yet. Cases 06–12 continue to add counterexample pressure: short working-state retention, static-array power boundaries, cache currentness, refresh-responsibility migration, condition-derived maintenance, external-versus-electrical erase authority, erase granularity, and endurance must all remain distinct.
