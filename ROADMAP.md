...
- [ ] latch / flip-flop / register — a bounded `first-pass` bridge exists in [`cases/06-flip-flop-powered-working-retention.md`](cases/06-flip-flop-powered-working-retention.md), with source-deepening ledgers [`evidence/06-flip-flop-register-boundary-addendum.md`](evidence/06-flip-flop-register-boundary-addendum.md) and [`evidence/06-eniac-timing-retention-deepening.md`](evidence/06-eniac-timing-retention-deepening.md). Eccles–Jordan GB148582A and the 1946 ENIAC report establish regenerative powered state, explicit set/reset/clear, short-lived working-state use, and ENIAC's own `stores` / `remembers` vocabulary. Whirlwind R-221 closes the period-`register` gap: it calls AR a `simple flip-flop register`, assigns it machine-level functions, and also uses `storage register` for core storage. ENIAC Part II p. IV-43 now directly supplies a two-flip-flop timing sequence in which an unsynchronized switch event is retained until central-programming pulses can turn it into a reliable synchronized transmitter action. Before marking the bridge complete, still directly inspect the 1919 Eccles–Jordan trigger-relay authorial page/reprint and directly render/inspect PX-1-105 / immediately relevant ENIAC schematic material; locator evidence and text-layer evidence do not substitute for visual circuit inspection;
- [ ] SRAM and cache;
...
**Current technical stress test:** [`cases/06-flip-flop-powered-working-retention.md`](cases/06-flip-flop-powered-working-retention.md) remains `first-pass`, now with [`evidence/06-flip-flop-register-boundary-addendum.md`](evidence/06-flip-flop-register-boundary-addendum.md) and [`evidence/06-eniac-timing-retention-deepening.md`](evidence/06-eniac-timing-retention-deepening.md). Its primary evidence supports short-duration retention, continuous-power/refresh separation, later state-sensitive use without a discrete retrieval transaction, a period architectural `register` boundary, and now a Part-II timing-retention sequence in which a pending unsynchronized event survives until machine timing can safely turn it into transmitter action.

**Next highest-value unit:** finish Case 06's two remaining direct-inspection gaps rather than opening another broad case. Directly inspect the 1919 Eccles–Jordan _Electrician_ p. 298 / _Radio Review_ pp. 143–146 page, and directly render/inspect ENIAC PX-1-105 `Flip-Flop Circuit` / immediately relevant drawing context. The period-`register` source gap and one machine-specific Part-II textual-operation gap are already closed and should not be researched again from scratch. Promote Case 06 only if the remaining primary visual material confirms the current mechanism reconstruction.
