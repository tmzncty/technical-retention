...
**Next highest-value unit:** move to one still-open Phase-2 bridge that changes the retention comparison rather than extending the now-grounded soft-updates slice. Prefer **RAID / scrubbing / rebuild** if period-primary evidence can cleanly separate redundancy, degraded operation, detection, reconstruction source, spare capacity, and rebuild completion; otherwise choose a bounded **write-back/dirty cache** or **NVMe persistence-domain** case. Do not turn Case 16 into a general history of journaling or modern `fsync` semantics.

The first generic thesis-audit sequence, four named philosophical/prior-art tests, the first category-coherence audit, and the post-audit technical stress tests are now grounded at bounded maturity. Do not promote the bounded relational criterion into a grand `What Is Technical Retention?` chapter yet. Cases 06–16 continue to add counterexample pressure: short working-state retention, static-array power boundaries, cache currentness, refresh-responsibility migration, condition-derived maintenance, external-versus-electrical erase authority, erase granularity, verification granularity, endurance, logical/physical disk indirection, payload-recovery separation, finite repair slack, volatile durability windows, explicit flush boundaries, failure-triggered emergency handoff, crash-admissible stable state, and relational `fsync` closure must all remain distinct.
