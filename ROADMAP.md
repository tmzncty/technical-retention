...
**Next highest-value synthesis unit:** a bounded Kirschenbaum forensic-materiality test beyond magnetic disk. Start from grounded mapped Flash and RADOS, where logical invalidation/currentness can diverge from physical survival, and test `erasure`, `variability`, `repeatability`, and `survivability` against remapping, obsolete-but-unerased Flash state, relocation, distributed copies, and — only where separately sourced — encryption/key-mediated disappearance. Preserve the distinction between interface disappearance and physical destruction rather than assuming either one entails the other.

The first generic thesis-audit sequence is complete, and three named philosophical/prior-art tests are now complete, but do not write a grand `What Is Technical Retention?` chapter yet. Continue testing named prior art against mechanism distinctions one claim at a time, preserving the counterexamples already recorded.
