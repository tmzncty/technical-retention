...
- [ ] SRAM and cache — the **static-MOS / SRAM substrate-array sub-slice is now `grounded`** in [`cases/07-static-mos-ram-powered-quiescence.md`](cases/07-static-mos-ram-powered-quiescence.md), with promotion record [`evidence/07-intel-pashley-1975-static-ram-grounding.md`](evidence/07-intel-pashley-1975-static-ram-grounding.md). The bounded source chain now includes period generic MOS cells, Intel product documentation, 5101L retention-mode evidence, and Pashley's Intel-assigned 1975 filing for a `1024 × 1`, `32 × 32`, +5 V static RAM with an explicitly bistable feedback-coupled cell and separate decode/sense/address-buffer machinery. This closes the broad manufacturer-primary Intel cell-mechanism gap without claiming that US3946369A is the 1101, 1101A, 2102, or 2102A. Exact product-topology genealogy, Vadasz 1971 facsimile inspection, and extra product-specific static-noise-margin work are archival/deepening tasks rather than promotion blockers. **Cache policy/interface semantics remain open and are now the next bounded sub-slice**; do not turn that work into another SRAM-cell survey;
...
**Grounded semiconductor stress test:** [`cases/07-static-mos-ram-powered-quiescence.md`](cases/07-static-mos-ram-powered-quiescence.md) is now `grounded`; see [`evidence/07-intel-pashley-1975-static-ram-grounding.md`](evidence/07-intel-pashley-1975-static-ram-grounding.md). The bounded source set supports period static-MOS / flip-flop / bistable vocabulary, no-refresh static operation, nondestructive read in cited Intel products, decoded-array organization, a 5101L low-voltage retention/operation-recovery boundary, and an Intel manufacturer-primary feedback-coupled static cell embedded in a decoded/sensed `1024 × 1` design. The source set also forces a new distinction: **cell-state retention ≠ reliable selection/sensing/recovery**. Exact commercial-product topology and the Vadasz facsimile remain archival cleanup, not central mechanism blockers.

**Next highest-value unit:** open a bounded **cache semantics** bridge without reopening generic SRAM history. The slice should ask what additional retention relation appears when a static-memory substrate becomes a cache: tag/data identity, hit/miss admissibility, replacement/invalidation, dirty state and write-back/write-through policy, and—only where historically sourced—coherence. It must separate the substrate's powered bistability from cache policy/interface semantics and should begin from period architecture/manual evidence rather than a modern textbook summary.

The first generic thesis-audit sequence, four named philosophical/prior-art tests, the first category-coherence audit, and the first two post-audit technical stress tests are now grounded at bounded maturity. Do not promote the bounded relational criterion into a grand `What Is Technical Retention?` chapter yet. Case 06 established later state-sensitive use, period register boundaries, pending-event retention, and state-holding versus transition/recovery separation; Case 07 now establishes the semiconductor-array continuation while separating held state from selection/sensing recoverability. Cache must test the next architectural layer rather than being forced into the substrate result.
