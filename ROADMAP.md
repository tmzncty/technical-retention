...
- [ ] ROM → PROM → EPROM → EEPROM → Flash — **EPROM sub-slice now grounded** in [`cases/11-intel-frohman-floating-gate-eprom-erasure.md`](cases/11-intel-frohman-floating-gate-eprom-erasure.md), with [`evidence/11-intel-1970-1971-floating-gate-eprom-grounding.md`](evidence/11-intel-1970-1971-floating-gate-eprom-grounding.md). The bounded Intel/Frohman sources separate trapped-charge nonvolatility, avalanche-injection programming, nondestructive lower-stress read, and radiation erase. The broad lineage remains unchecked because EEPROM electrical erasure and the later Flash erase-granularity transition still require separate period-primary cases;
...
The grounded EPROM bridge adds a different access/control boundary. An insulated floating gate can retain state without continuous operating power, X/Y electrical selection can program and nondestructively read cells, yet the bounded erase path can be radiation applied to the physical device rather than an ordinary addressed electrical command. Future semiconductor comparisons should therefore separate **hold**, **program**, **read**, and **erase** mechanisms and should treat **erase authority / erase geometry** as distinct from ordinary addressability. Later EEPROM and Flash cases are expected to change this partition rather than merely repeat `nonvolatile memory`.
...
**Next highest-value unit:** remain on the ROM/PROM/EPROM/EEPROM/Flash bridge and source one bounded **EEPROM electrical-erasure transition**. Prefer a manufacturer-primary or original-device source that directly shows how charge removal becomes electrically controlled and what selection/erase granularity it permits. Do not call this `Flash` merely because it is floating-gate and electrically erasable. Only after the EEPROM boundary is grounded should the next slice test the later Flash change in erase granularity / fast bulk erase, endurance, and maintenance semantics.

The first generic thesis-audit sequence, four named philosophical/prior-art tests, the first category-coherence audit, and the post-audit technical stress tests are now grounded at bounded maturity. Do not promote the bounded relational criterion into a grand `What Is Technical Retention?` chapter yet. Cases 06–11 continue to add counterexample pressure: short working-state retention, static-array power boundaries, cache currentness, refresh-responsibility migration, condition-derived maintenance, and trapped-charge erase asymmetry must all remain distinct.
