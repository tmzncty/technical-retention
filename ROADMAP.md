...
**Next highest-value unit:** continue Phase 2 with a bounded **DRAM evolution / refresh-machinery** slice only if it changes the retention comparison rather than repeating the grounded Case 03. A useful target would be a period-primary transition where refresh responsibility, row organization, self-refresh, controller integration, or error/recovery semantics materially change who or what performs retention work. If no such bounded change is sourceable in one pass, move to the ROM → PROM → EPROM → EEPROM → Flash bridge, where programmability and erase geometry clearly alter retention/write/forgetting semantics.

The first generic thesis-audit sequence, four named philosophical/prior-art tests, the first category-coherence audit, and the first three post-audit technical stress tests are now grounded at bounded maturity. Do not promote the bounded relational criterion into a grand `What Is Technical Retention?` chapter yet. Case 06 established later state-sensitive use, period register boundaries, pending-event retention, and state-holding versus transition/recovery separation; Case 07 established the semiconductor-array continuation while separating held state from selection/sensing recoverability; Case 08 now establishes derivative-copy currentness and cache-level admissibility while keeping authority and replacement distinct.
