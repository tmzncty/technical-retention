...
**Next highest-value synthesis unit:** a bounded **coherence / family-resemblance test** for the repository's own category `technical retention`. Do not begin with a definition and force the cases into it. Instead compare the five grounded regimes across the invariants that survived prior audits — state across temporal distance, recoverability relation, identity/currentness, maintenance trigger, embodiment replacement, and now trace survivability — and actively search for a counterexample showing that the category has become too broad. The output should decide whether one minimal operation survives all cases, whether several subfamilies are required, or whether `technical retention` should remain an explicitly heuristic umbrella.

The first generic thesis-audit sequence and four named philosophical/prior-art tests are now complete, but do not write a grand `What Is Technical Retention?` chapter yet. The next unit is still an adversarial boundary test of the project's own category, not a declaration that the synthesis has succeeded.
