...
- [ ] SRAM and cache — a bounded static-semiconductor sub-slice is now open as [`cases/07-static-mos-ram-powered-quiescence.md`](cases/07-static-mos-ram-powered-quiescence.md), currently **`first-pass`**. Period Vadasz–Chua–Grove (1971), NASA-CR-108672 (1970), and Intel 1975 documentation support static-MOS / flip-flop vocabulary, no-clock/no-refresh operation in the cited Intel devices, nondestructive read, decoded-array organization, and the 5101L distinction between ordinary operation and lower-voltage data retention. Cache policy/interface semantics remain intentionally deferred. Before promotion, directly inspect Vadasz 1971 pp. 43/47, obtain a cell-specific primary schematic/design source for an Intel 1101/1101A or 2102-class bit cell, and add device-specific hold/failure evidence; do not normalize the period evidence into a universal modern 6T-SRAM topology;
...
The first-pass static-MOS bridge adds another control: `power dependence` is not one Boolean property. Intel's 5101L documentation separates a lower-voltage state-retention condition from ordinary active operation and defines an operation-recovery relation between them, while 1101A/2102-class static memories still carry access/write timing despite needing no periodic refresh merely to remain. Future boundary work should therefore compare **retention-supporting supply**, **active-operation supply**, and **transition/recovery between power modes** rather than treating `volatile/static` or `needs power` as sufficient descriptors. Cache semantics remain a later layer.
...
**Grounded technical stress test:** [`cases/06-flip-flop-powered-working-retention.md`](cases/06-flip-flop-powered-working-retention.md) is now `grounded`; see [`evidence/06-burks-1947-eniac-flip-flop-grounding.md`](evidence/06-burks-1947-eniac-flip-flop-grounding.md). Its source set supports short-duration retention, continuous-power/refresh separation, later state-sensitive use without a discrete retrieval transaction, period register boundaries, staged pending-event retention, and a directly inspected period-published ENIAC circuit in which steady-state stability is explicitly separated from triggering/recovery. Exact PX-1-105 visual inspection remains archival cleanup for drawing-specific questions.

**Current semiconductor stress test:** [`cases/07-static-mos-ram-powered-quiescence.md`](cases/07-static-mos-ram-powered-quiescence.md) is now `first-pass`. It extends the powered-quiescent comparison into a period static-MOS array and has already forced a finer control: `static/no refresh` is not `unpowered`, and the 5101L shows that retention-supporting supply can differ from ordinary active-operation supply. Its package-level no-refresh, nondestructive-read, decoded-array, and low-voltage-retention claims are supported, while exact Intel bit-cell topology remains deliberately ungrounded.

**Next highest-value unit:** deepen Case 07 before opening cache semantics. Prioritize direct full-page inspection of Vadasz–Chua–Grove 1971 pp. 43/47, a cell-specific primary design/schematic source for Intel 1101/1101A or 2102-class static memory, and device-specific hold/noise-margin or failure evidence. Broad semiconductor-memory history should be linked to or developed in `computing-archaeology` rather than duplicated here.

The first generic thesis-audit sequence, four named philosophical/prior-art tests, the first category-coherence audit, and the first two post-audit technical stress tests are complete or open at bounded maturity. Do not promote the bounded relational criterion into a grand `What Is Technical Retention?` chapter yet. Case 06 established later state-sensitive use, period register boundaries, pending-event retention, and state-holding versus transition/recovery separation; Case 07 must now test those controls against integrated static semiconductor memory without importing cache policy or a universal modern SRAM topology.
