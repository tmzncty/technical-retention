...
**Next highest-value synthesis unit:** a bounded Heidegger availability/ordering test. Start from the already grounded addressability decomposition (`designation → selection/resolution → currentness/admissibility → recovery`) and ask whether modern technical ordering is clarified by those operations without collapsing `Bestand` into a noun for stored data. Use mapped Flash and RADOS to test callable/reorderable availability while preserving the counterexamples that addressability is not availability and physical survival is not currentness.
