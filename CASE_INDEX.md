...
90. **source locator ≠ direct source inspection** — recovering the exact 1919 Eccles–Jordan page and ENIAC PX-1-105 drawing identity improves source control but does not justify new quotation or component/topology claims until the page/schematic is directly inspected; evidence maturity remains `first-pass`;
...
**Current adversarial bridge:** [`cases/06-flip-flop-powered-working-retention.md`](cases/06-flip-flop-powered-working-retention.md) is still `first-pass`, now with [`evidence/06-flip-flop-register-boundary-addendum.md`](evidence/06-flip-flop-register-boundary-addendum.md) and [`evidence/06-eniac-timing-retention-deepening.md`](evidence/06-eniac-timing-retention-deepening.md). It supports short-lived working retention, separates continuous power from periodic state reconstruction, allows **state-sensitive use** rather than requiring explicit retrieval, has a period architectural `register` witness, and now directly anchors staged timing retention in ENIAC Part II p. IV-43.

**Next highest-value unit:** finish the two remaining direct visual/source inspections before opening the next semiconductor bridge. Directly inspect the 1919 Eccles–Jordan _Electrician_ p. 298 / _Radio Review_ pp. 143–146 page, and directly render/inspect ENIAC PX-1-105 `Flip-Flop Circuit` / immediately relevant drawing context. The former period-`register` gap and one machine-specific Part-II textual-operation gap are closed and should not be researched again from scratch. Promote Case 06 only if the remaining primary material confirms the current mechanism boundaries.