...
After seven bounded cases, **abacus/passive position, magnetic core, DRAM, mapped Flash, RADOS, and powered flip-flop are `grounded`; only delay line remains `first-pass`.** The repository satisfies both the numeric and mechanism-variety gates for bounded synthesis. This does **not** make the provisional theses conclusions: new technical bridges must remain free to break or revise the current relational criterion.
...
80. **category coherence is provisional and evidence-gated** — six grounded regimes now support the current relational criterion, including the powered flip-flop adversarial bridge; the first-pass delay-line case and future SRAM/cache/SSD/filesystem/RAID cases must still be allowed to break or revise it rather than being forced into it;
...
- [x] at least four contrasting cases at `grounded` or better — currently six;
...
**Grounded adversarial bridge:** [`cases/06-flip-flop-powered-working-retention.md`](cases/06-flip-flop-powered-working-retention.md) is now `grounded`, with the promotion record in [`evidence/06-burks-1947-eniac-flip-flop-grounding.md`](evidence/06-burks-1947-eniac-flip-flop-grounding.md). Burks 1947 pp. 757–759 directly supplies remembering-circuit vocabulary, a period-published simplified ENIAC schematic, the two-stable-state/DC-cross-coupling explanation, trigger/recovery separation, and microsecond-scale timing margins. Exact PX-1-105 visual inspection remains archival cleanup for original-drawing-specific claims rather than a generic mechanism blocker.

**Next highest-value unit:** open a bounded **SRAM / static semiconductor cell** bridge before adding cache semantics. Use period primary semiconductor-memory documentation to test whether the powered-quiescent distinction survives the substrate transition, and keep elementary cell stability, grouped memory organization, architectural register role, and cache policy/interface semantics separate. Reuse `computing-archaeology` for broad semiconductor-memory history rather than duplicating it.
