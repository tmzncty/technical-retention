...
def insert_after_paragraph(path: str, marker: str, paragraph: str) -> None:
    p = Path(path)
    text = p.read_text(encoding="utf-8")
    if paragraph.strip() in text:
        raise SystemExit(f"paragraph already present in {path}")
    start = text.find(marker)
    if start < 0:
        raise SystemExit(f"marker missing in {path}: {marker!r}")
    end = text.find("\n\n", start)
    if end < 0:
        raise SystemExit(f"paragraph end missing in {path}")
    p.write_text(text[:end] + "\n\n" + paragraph.strip() + text[end:], encoding="utf-8")
...
base_head = "2b9b3e5088f5ff8a9103fd0b57e7999294a0ad67"
doc_path = Path("docs/SYNTHESIS_18_CONSENSUS_SNAPSHOT_CONTINUATION.md")
if doc_path.exists():
    raise SystemExit(f"refusing to overwrite existing {doc_path}")

# Guard against running this one-shot integration on an unexpectedly old tree.
case_index = Path("CASE_INDEX.md").read_text(encoding="utf-8")
if "2092 — related-repository boundary" not in case_index:
    raise SystemExit("CASE_INDEX does not contain expected Synthesis 17 tail finding 2092")
if "2093 —" in case_index:
    raise SystemExit("CASE_INDEX already contains finding 2093; refusing to collide")

roadmap = Path("ROADMAP.md").read_text(encoding="utf-8")
open_question = "- [ ] In consensus snapshotting, how should committed command history, applied state, stable snapshot payload, log-position/term boundary metadata, cluster configuration, remaining log suffix, follower progress, and state-transfer repair be separated?"
if open_question not in roadmap:
    raise SystemExit("expected open consensus-snapshotting ROADMAP question is missing")


doc = r'''# Synthesis 18 — Consensus Snapshotting: History Replacement, Continuation Metadata, and State-Transfer Repair
...
readme_paragraph = r'''A bounded consensus-snapshotting comparison is now available in [`docs/SYNTHESIS_18_CONSENSUS_SNAPSHOT_CONTINUATION.md`](docs/SYNTHESIS_18_CONSENSUS_SNAPSHOT_CONTINUATION.md). Centered on grounded Raft Case 58 and stress-tested against GFS checkpointing, ZooKeeper fuzzy snapshots, and Bigtable materialization, it separates committed history, applied state, snapshot payload, continuation metadata, effective configuration, retained suffix, follower progress, replay/state-transfer repair, and later convergence. It fixes the counterexamples `snapshot payload ≠ complete continuation state`, `snapshot complete ≠ followers converged`, `history compacted ≠ history irrelevant`, and `one word snapshot ≠ one recovery contract`.'''
insert_after_paragraph(
    "README.md",
    "A bounded replicated-log layer comparison is now available in [`docs/SYNTHESIS_17_REPLICATED_LOG_SUFFIX_CURRENTNESS_VISIBILITY.md`]",
    readme_paragraph,
)

closed_question = r'''- [x] In consensus snapshotting, how should committed command history, applied state, stable snapshot payload, log-position/term boundary metadata, cluster configuration, remaining log suffix, follower progress, and state-transfer repair be separated? — bounded by [`docs/SYNTHESIS_18_CONSENSUS_SNAPSHOT_CONTINUATION.md`](docs/SYNTHESIS_18_CONSENSUS_SNAPSHOT_CONTINUATION.md): grounded Raft Case 58 supplies the canonical `committed/applied state → complete snapshot + lastIncludedIndex/Term + configuration → covered-prefix retirement → AppendEntries-or-InstallSnapshot` contract, while GFS Case 46, ZooKeeper Case 71, and Bigtable Case 57 provide counterexamples against universalizing `snapshot` semantics. The synthesis separates snapshot payload, continuation metadata, effective membership, retained suffix, follower progress, replay availability, state-transfer repair, and later convergence; broader checkpoint/consensus/state-machine-replication genealogy and production fault injection remain separate work for `computing-archaeology`.'''
replace_once("ROADMAP.md", open_question, closed_question)

findings = r'''
- **2093 — committed command history ≠ applied state:** in the bounded Raft case, commitment qualifies which history may enter the snapshot, while application is the later state-machine transition whose result is materialized. (`H/P` inherited, `E`)
- **2094 — applied state ≠ completed stable snapshot:** current state may already exist in the state machine while no finished replacement recovery image yet authorizes prefix retirement. (`H/P` inherited, `E`)
- **2095 — snapshot payload ≠ complete continuation state:** Raft retains last-included index/term and effective cluster configuration in addition to application-state payload. (`H/P` inherited, `E`)
- **2096 — boundary metadata ≠ replaced command history:** `lastIncludedIndex`/`lastIncludedTerm` locate the join between snapshot state and surviving lineage; they do not preserve the deleted command sequence. (`H/P` inherited, `E`)
- **2097 — configuration-entry retirement ≠ effective membership forgetting:** the current configuration must survive the compaction boundary even when the historical log entry that established it no longer does. (`H/P` inherited, `E`)
- **2098 — snapshot completion ≠ follower convergence:** a completed authoritative snapshot can permit local prefix retirement before lagging replicas have received that state and caught up. (`E`)
- **2099 — follower lag ≠ data loss:** lagging beyond ordinary replay reach can still be recoverable when a valid snapshot remains transferable. (`H/P` inherited, `E`)
- **2100 — replay availability ≠ state-transfer availability:** a follower may lose the ability to receive the missing command prefix while retaining a different recovery path through `InstallSnapshot`. (`H/P` inherited, `E`)
- **2101 — history-retirement frontier changes repair protocol:** compaction can preserve current-state recoverability while deliberately removing entry-by-entry reconstruction as a future option. (`E`)
- **2102 — longer surviving local history ≠ stronger recovery authority:** Raft snapshot installation can supersede physically surviving follower-local log history when the continuation boundary does not match. (`H/P` inherited, `E`, `A`)
- **2103 — checkpoint/snapshot presence ≠ recovery admissibility:** GFS explicitly skips incomplete checkpoints; readable bytes alone do not establish a valid recovery base. (`H/P` inherited, `E`)
- **2104 — snapshot ≠ exact historical instant universally:** ZooKeeper fuzzy snapshots may correspond to no point-in-time live tree while still forming a valid recovery base under ordered idempotent replay. (`H/P` inherited, `A`)
- **2105 — replay boundary ≠ maximum update already embodied in snapshot:** ZooKeeper's snapshot-start zxid can precede effects already present in the fuzzy image, so one boundary field cannot be universalized across snapshot designs. (`H/P` inherited, `A`)
- **2106 — one word `snapshot` ≠ one recovery contract:** Raft stable consensus snapshots, GFS checkpoints plus re-observation, ZooKeeper fuzzy images plus replay closure, and Bigtable SSTable materialization preserve different continuation relations. (`A`, `X`)
- **2107 — materialization prior art ≠ Raft genealogy:** Birrell 1987, Chubby 2006, GFS, ZooKeeper, and Bigtable constrain novelty claims for checkpoint/log/state materialization, but chronology and functional similarity do not prove direct implementation descent into Raft. (`H/P` inherited, `A`, `X`)
- **2108 — related-repository boundary:** current `tmzncty/computing-archaeology` search found no dedicated Raft/snapshot/checkpoint/consensus case to reuse; broad consensus, checkpoint, state-machine-replication, database-recovery, and implementation genealogy belongs there if developed. (`H/P` project-state record)
'''
append_once("CASE_INDEX.md", "2093 — committed command history", findings)

for path in [
    "docs/SYNTHESIS_18_CONSENSUS_SNAPSHOT_CONTINUATION.md",
    "README.md",
    "ROADMAP.md",
    "CASE_INDEX.md",
]:
...
# Final one-shot assertions before the workflow validates the git diff.
assert doc_path.exists()
assert "SYNTHESIS_18_CONSENSUS_SNAPSHOT_CONTINUATION.md" in Path("README.md").read_text(encoding="utf-8")
assert closed_question in Path("ROADMAP.md").read_text(encoding="utf-8")
idx = Path("CASE_INDEX.md").read_text(encoding="utf-8")
assert "2093 — committed command history ≠ applied state" in idx
assert "2108 — related-repository boundary" in idx
