...
EVIDENCE_PATH = Path("evidence/45-micron-ddr5-self-refresh-transition-state-horizon-deepening.md")
CASE_PATH = Path("cases/45-micron-ddr5-on-die-ecc-ecs.md")
ROADMAP_PATH = Path("ROADMAP.md")
INDEX_PATH = Path("CASE_INDEX.md")
SYNTHESIS_PATH = Path("docs/SYNTHESIS_26_MAINTENANCE_CONTROL_STATE_PERSISTENCE_HORIZONS.md")
WORKFLOW_PATH = Path(".github/workflows/integrate-case45-self-refresh-horizon.yml")
SELF_PATH = Path(".github/scripts/integrate-case45-self-refresh.py")

if EVIDENCE_PATH.exists():
    raise SystemExit("evidence file already exists; refusing duplicate integration")

evidence = r'''# Case 45 deepening — DDR5 ECS self-refresh continuity, PASR coverage, and transition-specific maintenance-state horizons
...
top_anchor = "Telemetry-validity deepening: [`../evidence/45-micron-ddr5-2022-2026-ecs-telemetry-validity-deepening.md`](../evidence/45-micron-ddr5-2022-2026-ecs-telemetry-validity-deepening.md)."
...
if top_anchor not in case:
    raise SystemExit("Case 45 top deepening anchor missing")
case = case.replace(top_anchor, top_add, 1)

case_marker = "\n## Maintenance and labor\n"
if case_marker not in case:
    raise SystemExit("Case 45 maintenance marker missing")
if "## Self-refresh transition horizons: preserved ECS state, reset refresh phase, and PASR validity" in case:
    raise SystemExit("Case 45 self-refresh deepening already integrated")
...
'''
case = case.replace(case_marker, "\n" + case_add.rstrip() + "\n" + case_marker, 1)
CASE_PATH.write_text(case, encoding="utf-8")

roadmap = ROADMAP_PATH.read_text(encoding="utf-8")
road_marker = "\n- [x] **Case 111 IBM ESS post-offline scrub completion:**"
if road_marker not in roadmap:
    raise SystemExit("Roadmap insertion marker missing")
if "Case 45 DDR5 ECS self-refresh transition-horizon deepening" in roadmap:
    raise SystemExit("Roadmap deepening already integrated")
road_add = r'''
- [x] **Case 45 DDR5 ECS self-refresh transition-horizon deepening:** [`cases/45-micron-ddr5-on-die-ecc-ecs.md`](cases/45-micron-ddr5-on-die-ecc-ecs.md) + [`evidence/45-micron-ddr5-self-refresh-transition-state-horizon-deepening.md`](evidence/45-micron-ddr5-self-refresh-transition-state-horizon-deepening.md) use Micron's Rev. D 10/2022 product-core contract, later Rev. E continuity, and bounded SK hynix product-document corroboration to separate self-refresh transition lifetime, REFsb enumeration phase, ECS schedule interval, ECS diagnostic state, and PASR coverage validity. Ordinary self-refresh entry/exit preserves ECS transparency counters/registers while the same transition resets Micron's REFsb internal bank counter; PASR can leave counter bits present while invalidating their full-array evidential meaning, requiring known-data initialization plus ECS counter reset before a later accurate full-array scrub. This closes `same transition != same maintenance-control-state persistence horizon`, `retained diagnostic bits != retained diagnostic validity`, and `self-refresh continuity != cross-power durability`. Direct JESD79-5 revision chronology, complete cross-vendor conformance, true power-removal persistence, and independent fault/power-transition validation remain open, with broad DDR5 mechanism genealogy routed to `computing-archaeology`.
'''
roadmap = roadmap.replace(road_marker, "\n" + road_add.rstrip() + road_marker, 1)
ROADMAP_PATH.write_text(roadmap, encoding="utf-8")

index = INDEX_PATH.read_text(encoding="utf-8")
if "**3700 —" in index:
    raise SystemExit("CASE_INDEX finding 3700 already exists; possible concurrent integration")
if "**3699 — X**" not in index:
    raise SystemExit("CASE_INDEX latest expected finding 3699 missing; possible concurrent integration")
old_row = "| [Micron DDR5 On-Die ECC and Error Check Scrub: Read Correction, Array Repair, and Error Transparency](cases/45-micron-ddr5-on-die-ecc-ecs.md) | **grounded** | commercial DDR5 dynamic array + on-die SEC code + read-path correction + manual/automatic ECS corrective writeback + error-count evidence + separate ordinary refresh | separate correct logical read from repaired stored embodiment; ECS scrub from DRAM refresh; successful payload delivery from error observability; and device-internal automation from later host/platform policy | [2021–2026 ODECC/ECS grounding](evidence/45-micron-ddr5-2021-2026-odecc-ecs-grounding.md); direct revision-by-revision JESD79-5 audit, independent product fault validation, system-level ECC/PPR/sparing composition, and RowHammer interaction remain separate work |"
new_row = "| [Micron DDR5 On-Die ECC and Error Check Scrub: Read Correction, Array Repair, and Error Transparency](cases/45-micron-ddr5-on-die-ecc-ecs.md) | **grounded** | commercial DDR5 dynamic array + on-die SEC code + read-path correction + manual/automatic ECS corrective writeback + error-count evidence + transition-specific maintenance-control state + separate ordinary refresh | separate correct logical read from repaired stored embodiment; ECS scrub from DRAM refresh; successful payload delivery from error observability; diagnostic-state survival from diagnostic validity; and device-internal automation from later host/platform policy | [2021–2026 ODECC/ECS grounding](evidence/45-micron-ddr5-2021-2026-odecc-ecs-grounding.md) + [2022 ECS telemetry-validity deepening](evidence/45-micron-ddr5-2022-2026-ecs-telemetry-validity-deepening.md) + [2022–2024 self-refresh/PASR transition-horizon deepening](evidence/45-micron-ddr5-self-refresh-transition-state-horizon-deepening.md); direct revision-by-revision JESD79-5 audit, true power-removal telemetry persistence, independent product fault validation, system-level ECC/PPR/sparing composition, and RowHammer interaction remain separate work |"
if old_row not in index:
    raise SystemExit("Case 45 CASE_INDEX row anchor missing")
index = index.replace(old_row, new_row, 1)

findings = r'''

### Findings 3700–3715 — Case 45 DDR5 ECS self-refresh transition horizons and PASR validity

- **3700 — H/P** — Micron Rev. D 10/2022 states that entering and exiting self refresh does not reset the ECS transparency counters/registers.
- **3701 — H/P** — Micron requires automatic ECS work to continue during self refresh under the documented enable conditions, while warning that the internal scrub rate can vary because operation synchronizes to an internal oscillator.
- **3702 — H/P** — Micron allows the maximum-spacing interval for later REFab commands or another self-refresh entry to restart upon self-refresh exit even though ECS transparency counters/registers remain intact.
- **3703 — H/P** — In the same product-core document, entering and exiting self refresh resets the internal bank counter used by Same Bank Refresh coverage.
- **3704 — H/P** — If all banks were not covered by REFsb before self-refresh entry, Micron prescribes an extra REFab or an extra REFsb to each bank after exit, separating phase reset from an automatic assertion of payload loss.
- **3705 — H/P** — Under PASR, masked segments are not guaranteed to retain data and the DRAM is not required to execute ECS on them while in self refresh.
- **3706 — H/P** — Micron warns that ECS transparency may be inaccurate when PASR mask bits are set; after exit, known-data initialization of masked segments plus ECS-counter reset is required before treating the next full-array scrub as accurate.
- **3707 — H/P** — Device RESET / ECS RESET COUNTERS remains a distinct ECS-state boundary that initializes ECS counters/address state and resets MR16–MR20 rather than merely crossing self-refresh mode.
- **3708 — H/P*** — Public SK hynix DDR5 product documents independently expose automatic ECS in self refresh plus explicit RESET/ECS-reset initialization, and one product-family document likewise states that self-refresh entry/exit does not reset ECS transparency counters/registers; this is bounded product-document corroboration, not a complete vendor matrix.
- **3709 — E** — The same self-refresh transition can preserve ECS diagnostic state while resetting REFsb enumeration phase; therefore `same device transition != same persistence horizon for every maintenance-control state`.
- **3710 — E** — Restarting an ECS scheduling interval without clearing transparency state shows `maintenance schedule phase != diagnostic/report state`.
- **3711 — E** — PASR supplies the counterexample `retained diagnostic bits != retained diagnostic validity`: a summary can survive while its covered measurement population changes enough to invalidate a full-array interpretation.
- **3712 — E** — `maintenance-coverage phase reset != payload reset != diagnostic-summary reset`; compensating refresh can re-establish coverage after REFsb phase is reinitialized.
- **3713 — A/E** — Cases 09 and 21 are functional comparison points for counter-phase reinitialization and self-refresh responsibility handoff, but neither establishes a genealogy into DDR5 ECS behavior.
- **3714 — X** — ECS transparency surviving self refresh does not establish power-loss persistence or nonvolatility of MR16–MR20; the power-removal boundary remains open.
- **3715 — X** — Similar Micron/SK hynix product behavior does not establish universal JESD79-5 revision identity, invention priority, deterministic PASR data loss, or measured silicon behavior under every transition/fault.
'''
index = index.rstrip() + findings + "\n"
INDEX_PATH.write_text(index, encoding="utf-8")

synthesis = SYNTHESIS_PATH.read_text(encoding="utf-8")
syn_anchor = "The project term `reporting/configuration epoch` is used only to describe the validity interval over which a count mode/threshold and its accumulated summary can be interpreted together. It is not Micron or JEDEC historical vocabulary, and this synthesis does not claim the reporting registers survive power removal."
if syn_anchor not in synthesis:
    raise SystemExit("Synthesis 26 Case45 anchor missing")
syn_add = syn_anchor + r'''
...
synthesis = synthesis.replace(syn_anchor, syn_add, 1)

rej_anchor = "- **`payload survival implies control-state survival` — rejected.** A physical replica can survive while a restarted NameNode has not yet re-observed it."
if rej_anchor not in synthesis:
    raise SystemExit("Synthesis 26 rejected-claims anchor missing")
synthesis = synthesis.replace(rej_anchor, rej_anchor + "\n- **`one transition gives every maintenance-control state the same lifetime` — rejected.** DDR5 self-refresh entry/exit preserves ECS transparency state while resetting REFsb bank phase, and PASR can preserve bits while invalidating their prior full-array evidential scope.", 1)
