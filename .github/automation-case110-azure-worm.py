...
EVIDENCE_PATH = Path('evidence/110-azure-2018-2026-immutable-blob-worm-deepening.md')
EVIDENCE = r'''# Evidence 110 deepening — Azure Immutable Blob Storage, 2018 cloud-WORM prior-art floor and later version-level policy semantics
...
if 'Azure Immutable Blob Storage: a closer same-class prior-art floor' not in case:
    old_scope = "- **Prior-art floor:** ECMA-153's June 1994 second edition standardizes a magneto-optical Write Once, Read Multiple (WORM) cartridge. It is used only to block an S3-first/WORM-invention claim, not to assert a direct engineering genealogy."
    new_scope = "- **Prior-art floors:** ECMA-153's June 1994 second edition standardizes a magneto-optical WORM cartridge; Microsoft Azure Immutable Blob Storage then supplies a much closer cloud-service floor, with public preview on 19 June 2018 and general availability on 18 September 2018, before AWS's 26 November 2018 Object Lock launch. Neither chronology establishes invention priority or direct genealogy."
    assert old_scope in case
    case = case.replace(old_scope, new_scope, 1)
    marker = '## Engineering reconstruction\n'
    insert = r'''## Cross-provider historical deepening
...
'''
    ...
    case = case.replace(marker, insert + marker, 1)
    old_no = '- provider-independent equivalence with Azure, GCS, or on-premises object-lock systems;'
    new_no = '- provider-independent equivalence across Azure, GCS, or on-premises object-lock systems; this pass establishes only specific Azure/S3 chronology and contract contrasts, not implementation equivalence;'
    assert old_no in case
    case = case.replace(old_no, new_no, 1)
    old_open = '- Compare named provider/object-store WORM contracts without flattening version scope, bypass authority, default retention, and legal holds.'
    new_open = '- Extend the now-grounded Azure comparison to GCS and/or on-premises object-lock systems without flattening version scope, bypass authority, default retention, legal holds, or implementation lineage.'
    assert old_open in case
    case = case.replace(old_open, new_open, 1)
    old_sources = 'See [`evidence/110-amazon-s3-2018-object-lock-grounding.md`](../evidence/110-amazon-s3-2018-object-lock-grounding.md).'
    new_sources = 'See [`evidence/110-amazon-s3-2018-object-lock-grounding.md`](../evidence/110-amazon-s3-2018-object-lock-grounding.md) and [`evidence/110-azure-2018-2026-immutable-blob-worm-deepening.md`](../evidence/110-azure-2018-2026-immutable-blob-worm-deepening.md).'
    assert old_sources in case
    case = case.replace(old_sources, new_sources, 1)
    case_path.write_text(case, encoding='utf-8')

roadmap_path = Path('ROADMAP.md')
roadmap = roadmap_path.read_text(encoding='utf-8')
roadmap_entry = "- [x] Case 110 Azure Immutable Blob Storage cross-provider WORM prior-art / policy-granularity deepening — [`cases/110-amazon-s3-object-lock-version-worm-retention.md`](cases/110-amazon-s3-object-lock-version-worm-retention.md), deepened by [`evidence/110-azure-2018-2026-immutable-blob-worm-deepening.md`](evidence/110-azure-2018-2026-immutable-blob-worm-deepening.md): Microsoft first-party records place Azure Immutable Blob Storage in public preview on 19 June 2018 and GA on 18 September 2018, before AWS's 26 November 2018 Object Lock launch, supplying a closer same-class public cloud-WORM prior-art floor while preserving `chronology != invention/genealogy`. The 2018 Azure witness is container-scoped; Blob versioning GA in August 2020 plus the current `2020-06-12` REST floor bound later version-level WORM semantics. Current Azure further separates capability enablement, inherited/default policy, per-version protection, legal hold, retention deadline, and one-way policy locking, while `Azure locked/unlocked != S3 Governance/Compliance` and service-level WORM remains distinct from physical write-once media. GCS/on-prem comparison, full Azure revision chronology, geo-failover policy-state behavior, backend embodiment/sanitization, and fault validation remain open."
if roadmap_entry not in roadmap:
    prefix = '- [x] Amazon S3 Object Lock per-version WORM / retention-authority boundary'
    pos = roadmap.find(prefix)
    assert pos >= 0
    end = roadmap.find('\n', pos)
    assert end >= 0
    roadmap = roadmap[:end+1] + roadmap_entry + '\n' + roadmap[end+1:]
    roadmap_path.write_text(roadmap, encoding='utf-8')

index_path = Path('CASE_INDEX.md')
index = index_path.read_text(encoding='utf-8')
row_prefix = '| [Amazon S3 Object Lock: Per-Version WORM Retention, Legal Holds, and Delete-Marker Boundaries]'
row_start = index.find(row_prefix)
assert row_start >= 0
row_end = index.find('\n', row_start)
assert row_end >= 0
new_row = "| [Amazon S3 Object Lock: Per-Version WORM Retention, Legal Holds, and Delete-Marker Boundaries](cases/110-amazon-s3-object-lock-version-worm-retention.md) | **grounded** | versioned payload + version-scoped Governance/Compliance retention + retain-until metadata + independent legal-hold status + delete-marker/currentness relation + Lifecycle interaction + bounded Azure cross-provider policy comparison | separate version retention from immutable protection; protected version from frozen key; time expiry from deletion; Governance bypass from Compliance; legal hold from retention period; service-level WORM from physical write-once media; same-class prior-art chronology from contract equivalence | [2018 launch + current-contract grounding](evidence/110-amazon-s3-2018-object-lock-grounding.md) + [Azure 2018→current cross-provider WORM deepening](evidence/110-azure-2018-2026-immutable-blob-worm-deepening.md); Azure now establishes an earlier same-class public cloud-WORM floor and a distinct later version-level policy state machine; GCS/on-prem comparison, full chronology, backend embodiment/sanitization, compliance-law analysis, and fault validation remain separate work |"
index = index[:row_start] + new_row + index[row_end:]

if '## Case 110 deepening — Azure cross-provider WORM findings' not in index:
    findings = r'''

## Case 110 deepening — Azure cross-provider WORM findings

Deepening record: [`evidence/110-azure-2018-2026-immutable-blob-worm-deepening.md`](evidence/110-azure-2018-2026-immutable-blob-worm-deepening.md).

- **3049 — Azure public preview != Azure general availability:** Microsoft dates Immutable Blob Storage public preview to 19 June 2018 and GA to 18 September 2018; public disclosure and general service availability are separate chronology claims. (`H/P`, `X`)
- **3050 — Azure September-2018 GA predates the dated S3 Object Lock public launch:** Microsoft's GA record precedes AWS's 26 November 2018 launch, supplying a near-contemporary cloud-service WORM prior-art floor rather than only an older physical-media analogy. (`H/P`)
- **3051 — earlier Azure public cloud WORM != Azure invention priority or AWS genealogy:** the chronology blocks an S3-first public-service shortcut but does not establish first invention, private-development priority, copying, or causal descent. (`H/P`, `X`)
- **3052 — Azure 2018 container-level WORM != current Azure version-level WORM:** the 2018 provider record applies policy at Blob Container scope, while current per-version semantics belong to a later versioning/API layer. (`H/P`, `X`)
- **3053 — Blob versioning GA != 2018 WORM GA:** Microsoft separately dates Blob versioning GA to August 2020; retaining previous versions and enforcing WORM policy are distinct capabilities and chronologies. (`H/P`, `E`)
- **3054 — REST `2020-06-12` version-policy floor != evidence of 2018 per-version semantics:** the current Set Blob Immutability Policy contract explicitly dates snapshot/version support to that API version and therefore helps prevent backward projection. (`H/P`, `X`)
- **3055 — version-level WORM support enabled != active version protection:** current Azure separates enabling/migrating WORM capability from actually setting a time-based policy or legal hold. (`H/P`, `E`)
- **3056 — irreversible capability enablement != every payload permanently immutable:** current Azure says version-level WORM support cannot be removed from an enabled container, while actual blob/version protection still depends on policy/hold state. (`H/P`, `E`, `X`)
- **3057 — current default policy != retroactive rewrite of inherited version policy:** changing an account/container default does not alter policies already inherited by existing blob versions. (`H/P`, `E`)
- **3058 — retention deadline != policy mutability:** current Azure's locked state can preserve the rule against shortening/deleting a policy while the retain-until time remains a separate value. (`H/P`, `E`)
- **3059 — `unlocked -> locked` != reversible mode toggle:** the current REST contract permits locking but not unlocking, so policy-authority state itself has a one-way transition. (`H/P`, `E`)
- **3060 — protected version != frozen logical blob name:** current version-level Azure permits replacement-style operations because they create a new version; an older protected version may persist while name-level currentness advances. (`H/P`, `E`)
- **3061 — Azure version semantics != S3 delete-marker semantics:** Azure's current documentation describes a deleted current version becoming a previous version; that does not establish an S3-style data-less delete marker in Azure. (`H/P`, `A`, `X`)
- **3062 — Azure `locked/unlocked` != S3 Governance/Compliance:** both govern WORM-related authority, but their documented transitions, inheritance, and bypass models differ; no one-to-one mode translation is established. (`A`, `X`)
- **3063 — cross-provider service-level WORM != physically write-once medium:** both provider contracts constrain service operations while leaving backend replica movement, media rewrite, key retirement, reclamation, and sanitization unproven. (`E`, `A`, `X`)
- **3064 — related-repository boundary:** a fresh `tmzncty/computing-archaeology` search found no dedicated Azure Blob immutability/WORM study; broad cloud-WORM and provider-implementation genealogy belongs there if developed, while this deepening keeps only the retention-policy chronology and relation comparison. (`H/P` project-state record)
'''
    index = index.rstrip() + findings + '\n'
index_path.write_text(index, encoding='utf-8')

for temp in [Path('.github/automation-case110-azure-worm.py'), Path('.github/workflows/tmp-case110-azure-worm-deepening.yml')]:
    if temp.exists():
        temp.unlink()
